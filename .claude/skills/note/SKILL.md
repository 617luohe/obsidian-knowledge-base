...
当在非 vault 项目中调用 note 时：
- 自动执行 noteall 环境检测（见 noteall §〇）
- 外部项目模式下，所有文件操作以默认知识库为根，无需用户感知路径切换
- 信号词：`保存到知识库`、`记到默认知识库`、`笔记同步到 vault` — 显式触发外部路由
- 如想在本项目创建本地笔记 → 明确说"在当前项目创建"、"本地笔记"
