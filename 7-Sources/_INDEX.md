---
title: "7-Sources 索引"
tags: [type/moc, system/index]
created: 2026-07-27
updated: 2026-07-27
auto-indexed: 2026-07-27 16:00
---

# 📂 7-Sources — 来源

> 外部知识来源的管理中心。书籍、文章、课程、视频等外部内容的元数据和笔记。
> 🤖 自动索引 | 最后更新: 2026-07-27 | [[🏠 知识库入口|返回首页]]

## 📊 统计

| 指标 | 值 |
|------|-----|
| 来源总数 | 47 |
| 子项目数 | 3 |
| 最近新增（7天） | 14 |
| 需关注 | 0 |

## 📑 来源目录

<!-- INDEX-KEEPER-MANAGED:START -->

### 个人知识
- [[Vegas 交易系统（罗晟）]] — 交易系统来源
- [[PyCharm 激活注册（ckey.run）]] — 工具激活参考
- [[半年工作总结-陈昱慧]] — 工作总结文档
- [[2026年上半年工作总结.pptx]] — 工作总结 PPT 原件（已归档）
- [[Q2-2026-工作汇报-李涵彬]] — 工作汇报文档

### skills-legacy — my-skills 旧版归档（14 篇）
- [[skills-legacy/MOC-my-skills 使用指南]] — 旧版 skills 使用指南总览
- [[skills-legacy/skill-0 通用帮助]] — skill-0 帮助文档
- [[skills-legacy/skill-1 Claude.md 管理]] — skill-1 文档
- [[skills-legacy/skill-2 Obsidian 知识库]] — skill-2 文档
- [[skills-legacy/skill-3 会议纪要]] — skill-3 文档
- [[skills-legacy/skill-4 日记与回顾]] — skill-4 文档
- [[skills-legacy/skill-5 内容创作]] — skill-5 文档
- [[skills-legacy/skill-6 批量整理]] — skill-6 文档
- [[skills-legacy/skill-7 任务拆解]] — skill-7 文档
- [[skills-legacy/skill-8 项目交付]] — skill-8 文档
- [[skills-legacy/skill-9 收尾归档]] — skill-9 文档
- [[skills-legacy/use-skills 智能路由]] — 路由机制文档
- *等 14 篇*（2026-07-27 归档）

### grooming — 个人护理参考（1 篇）
- [[grooming/RefinedShave 刀片锋利度测试数据]] — 刀片测试数据

### 热轧排程项目 — 项目知识库（32 篇）
```
热轧排程项目/
├── [[热轧排程项目/README]] — 项目入口
├── 01-快速入口/ — 场景导航、术语表、接口契约（4 篇）
├── 02-系统架构/ — 调用链、配置、接口文档（4 篇）
├── 03-粗排算法/ — 算法设计、规则说明（5 篇）
├── 04-评分模块/ — PRD、任务拆解（5 篇）
├── 05-排障与调试/ — 排障指南、调试清单（3 篇）
├── 06-数据库与结果存储/ — 表结构、存储方案（5 篇）
└── 07-项目周报/ — 周报与 CSV 数据
```

<!-- INDEX-KEEPER-MANAGED:END -->

<!-- MANUAL:START -->
<!-- 以下为用户手动添加的内容，index-keeper 不会修改 -->
<!-- MANUAL:END -->

## 🔧 维护日志

| 日期 | 变更 | 触发 |
|------|------|------|
| 2026-07-27 | 初始创建，收录 47 篇来源（3 个子项目） | index-keeper 初始化 |
